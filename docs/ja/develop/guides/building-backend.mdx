...
<Steps>
  <Step title="Pythonパッケージを追加する">
    Morphは一般的なPythonプロジェクトの形式であるため、自由にPythonパッケージを追加することができます。`requirements.txt`や`poetry.toml`に必要なパッケージを追加してください。
  </Step>
...
LLM APIなどのストリーミングAPIを使用する場合に使用します。 `stream_chat` 関数を用いて値の流し込みを実行します。
