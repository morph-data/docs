...
<Tabs>
  ...
  <Tab title="Local Version">
    <Info>For the local version, you need to directly write the credentials in `~/.morph/connections.yml`. For other methods, please create them in the cloud version.</Info>
    ...
    The access_token refresh is a cloud version feature, so the one specified in the local environment will always be used.
