...
<Tabs>
  ...
  <Tab title="Local Version">
    ...
    The access_token refresh is a cloud version feature, so the one specified will always be used in the local environment.

    <Warning>
        When actually specifying the connector and executing the file, the priority is as follows:
        Also, note that only connectors created in the cloud version can be used in environments where `morph deploy` is performed.
